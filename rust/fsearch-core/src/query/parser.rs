...
impl Parser {
    ...
    fn apply_modifiers(&mut self, node: Node) -> Node {
        let mut mods = Vec::new();
        loop {
            let (next_tok, next_text) = self.lexer.peek_token();
            if let Token::Word = next_tok {
                if let Some(t) = next_text {
                    if t.starts_with('@') {
                        self.advance();
                        mods.push(t.trim_start_matches('@').to_string());
                        continue;
                    }
                }
            }
        ...
        if mods.is_empty() { node } else { Node::Modified(Box::new(node), mods) }
    }
    ...
    fn parse_primary(&mut self) -> Option<Node> {
        match &self.cur {
                ...
                match &self.cur {
                    Token::Word(w) => {
                        ...
                        if term.contains("..") {
                            ...
                            let low = parts.get(0).and_then(|s| if s.is_empty() { None } else { Some(Bound::Inclusive(s.to_string())) }).unwrap_or(Bound::Unbounded);
                            let high = parts.get(1).and_then(|s| if s.is_empty() { None } else { Some(Bound::Inclusive(s.to_string())) }).unwrap_or(Bound::Unbounded);
                            self.advance();
                            return Some(self.apply_modifiers(Node::Range(field_name, low, high)));
                        }
                        self.advance();
                        Some(self.apply_modifiers(Node::Field(field_name, term)))
                    }
                    _ => None,
                }
            }
            Token::FieldEmpty(name) => {
                let field_name = name.clone();
                self.advance();
                Some(self.apply_modifiers(Node::Field(field_name, String::new())))
            }
            Token::Word(name) => {
                let name_clone = name.clone();
                // function call detection
                let (next_tok, _next_text) = self.lexer.peek_token();
                if let Token::BracketOpen = next_tok {
                    ...
                    if let Token::BracketClose = self.cur { self.advance(); }
                    return Some(self.apply_modifiers(Node::Function(name_clone, args)));
                }

                // comparisons like size<1000 without colon
                let (next_tok, _next_text) = self.lexer.peek_token();
                match next_tok {
                    Token::Smaller => {
                        self.advance(); self.advance();
                        if let Token::Word(v) = &self.cur { let val = v.clone(); self.advance(); return Some(self.apply_modifiers(Node::Compare(name_clone, CompareOp::Smaller, val))); }
                        return None;
                    }
                    Token::SmallerEq => {
                        self.advance(); self.advance();
                        if let Token::Word(v) = &self.cur { let val = v.clone(); self.advance(); return Some(self.apply_modifiers(Node::Compare(name_clone, CompareOp::SmallerEq, val))); }
                        return None;
                    }
                    Token::Greater => {
                        self.advance(); self.advance();
                        if let Token::Word(v) = &self.cur { let val = v.clone(); self.advance(); return Some(self.apply_modifiers(Node::Compare(name_clone, CompareOp::Greater, val))); }
                        return None;
                    }
                    Token::GreaterEq => {
                        self.advance(); self.advance();
                        if let Token::Word(v) = &self.cur { let val = v.clone(); self.advance(); return Some(self.apply_modifiers(Node::Compare(name_clone, CompareOp::GreaterEq, val))); }
                        return None;
                    }
                    Token::Equal => {
                        self.advance(); self.advance();
                        if let Token::Word(v) = &self.cur { let val = v.clone(); self.advance(); return Some(self.apply_modifiers(Node::Compare(name_clone, CompareOp::Eq, val))); }
                        return None;
                    }
                    Token::Contains => {
                        self.advance(); self.advance();
                        if let Token::Word(v) = &self.cur { let val = v.clone(); self.advance(); return Some(self.apply_modifiers(Node::Compare(name_clone, CompareOp::Contains, val))); }
                        return None;
                    }
                    _ => {
                        ...
                        if s.len() >= 2 && s.starts_with('/') && s.ends_with('/') {
                            ...
                            self.advance();
                            return Some(self.apply_modifiers(Node::Regex(pat)));
                        }
                        self.advance();
                        return Some(self.apply_modifiers(Node::Word(s)));
                    }
                }
            }
            _ => None,
...
mod tests {
    ...
    fn parse_basic_expression() {
        ...
        match ast.unwrap() {
            Node::And(left, right) => {
                match *left {
                    Node::Field(ref n, ref v) => { assert_eq!(n, "name"); assert_eq!(v, "foo"); }
                ...
                match *right {
                    Node::Group(boxed) => {
                        match *boxed {
                            Node::Or(a, b) => {
                                match *a { Node::Word(ref w) => assert_eq!(w, "bar"), _ => panic!("expected bar"), }
                                match *b { Node::Word(ref w) => assert_eq!(w, "baz"), _ => panic!("expected baz"), }
                            }
